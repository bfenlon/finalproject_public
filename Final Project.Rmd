...
```{r}
# Run paired t-tests on both models between time periods 
# (paired because these are two observations of the same area from two different time periods)
...
long_before_after_blob <- before_after_blob %>% 
  pivot_longer(c(before, after), names_to = 'time.period', values_to = 'textblob.sentiment')
